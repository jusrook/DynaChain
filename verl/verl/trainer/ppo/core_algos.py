...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
@register_adv_est(AdvantageEstimator.CUSTOM)  # or simply: @register_adv_est("custom")
def compute_grpo_custom_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute advantage for custom_GRPO, operating only on Outcome reward
    (with only one scalar reward for each response).
...
def agg_loss_weighted(
    loss_mat: torch.Tensor,
    mask: torch.Tensor,
    old_log_prob: torch.Tensor,
    advantages: torch.Tensor,  # 接收 A
    tau: float = 0.0
):
    """
    根据 A (advantages) 的符号非对称地加权 loss。
    tau_eff = tau * sign(A)
    """
    mask = mask.bool()
    if tau == 0.0:
    ...
    # 1. 计算信息量
    info = -old_log_prob.detach()  # [B, S]

    # 2. 计算每个 token 的有效 tau (tau_eff)
    # torch.sign(0) = 0, 为避免 A=0 时 tau_eff=0, 我们用 where
    adv_sign = torch.where(advantages >= 0, 1.0, -1.0) # [B, S]
    tau_eff = tau * adv_sign                           # [B, S]

    # 3. 计算 softmax 的输入
    # 核心逻辑：info * tau_eff
    softmax_input = info * tau_eff                     # [B, S]

    # 4. 掩码 (Masking)
    # 这是最干净的掩码方式：
    # 无论 tau_eff[i,j] 是正还是负，
    # 我们都希望 padding token 在 softmax 之前是 -inf，这样它的权重就是 0。
    softmax_input = softmax_input.masked_fill(~mask, -torch.inf)

    # 5. 计算权重
    w = F.softmax(softmax_input, dim=-1)      # [B, S]
    # w = w.masked_fill(~mask, 0.0) # 理论上不再需要，但保留也无害

    # 6. 计算最终 loss
    loss_mat = torch.where(mask, loss_mat, 0.0) # 保持 NaN/inf 保护
    return (loss_mat * w).sum(dim=-1).mean()

def agg_loss_weighted_batch(
    loss_mat: torch.Tensor,
    mask: torch.Tensor,
    old_log_prob: torch.Tensor,
    advantages: torch.Tensor,
    ...
    """
    在 batch 层次计算非对称加权损失（权重在整个 batch 的有效 token 上归一化）

    Args:
        loss_mat: (B, S)
        mask: (B, S)  bool
        old_log_prob: (B, S)
        advantages: (B, S)  每个 token 的 advantage
        tau: float  探索-利用系数（[-1.0, 1.0]）
    ...
    # 4. 展平 → 全局 softmax
    flat_input = softmax_input.view(-1)         # (N,)
    flat_w = F.softmax(flat_input, dim=0)       # 全局归一化权重

    # 5. 重塑并加权
    w = flat_w.view(loss_mat.shape)             # (B, S)
    loss_masked = torch.where(mask, loss_mat, 0.0)
    loss = (loss_masked * w).sum()              # 已归一化，直接求和
...
def compute_policy_loss_vanilla(
    ...
    # rank = torch.distributed.get_rank()          # 当前进程全局 rank
    # debug_rank = int(os.environ.get("VERL_DEBUG_RANK", 0))
    # debug_port = int(os.environ.get("VERL_DEBUG_PORT", 5678))

    # if rank == debug_rank and debug_port > 0:
    #     debugpy.listen(("0.0.0.0", debug_port))
    #     print(f"[rank {rank}] Waiting for debugger on port {debug_port} ...")
    #     debugpy.wait_for_client()          # 阻塞直到 VSCode/PyCharm attach
    #     debugpy.breakpoint()               # 可选：直接停在这里
    assert config is not None
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    if config.token_weighted:
        # print('use token weighted')
        tau = config.get('token_weighted_temperature',1.0)
        pg_loss = agg_loss_weighted_batch(loss_mat=pg_losses, mask=response_mask, old_log_prob=old_log_prob, advantages=advantages, tau=tau)
    else:
        # print('use token mean')
        pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
